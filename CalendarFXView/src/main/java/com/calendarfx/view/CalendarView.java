...
/**
 * A full calendar view with multiple pages for displaying a single day, a week, a month, and an entire year. The view
 * also includes two trays, one for seeing all calendar sources, and one for seeing the results of the current search.
 * The trays can be shown or hidden. Another nice feature is direct support for printing (see {@link #getPrintView()}).
 * <p/>
...
public class CalendarView extends DateControl {
  ...
  public CalendarView() {
    ...
    /*
     * We do have a user agent stylesheet, but it doesn't seem to work properly when run as a standalone jar file.
     */
    getStylesheets().add(CalendarView.class.getResource("calendar.css") //$NON-NLS-1$
        .toExternalForm());

    /*
     * We are "abusing" the properties map to pass new values of read-only properties from the skin to the control.
     */
    getProperties().addListener((MapChangeListener.Change<? extends Object, ? extends Object> change) -> {
      if (change.getKey()
          .equals(SELECTED_PAGE)) {
    ...
    dayPage.hiddenProperty()
        .addListener(fixSelectedPageListener);
    weekPage.hiddenProperty()
        .addListener(fixSelectedPageListener);
    monthPage.hiddenProperty()
        .addListener(fixSelectedPageListener);
    yearPage.hiddenProperty()
        .addListener(fixSelectedPageListener);
  ...
  /**
   * Returns the developer console that can be made visible via a META-D key stroke when the system property
   * "calendarfx.developer" is set to true.
   *
   * @return the developer console or null if the system property "calendarfx.developer" is not set to true
   */
  ...
  /**
   * Returns the print view associated with this view. The print view shows a print result preview dialog that allows
   * the user to customize several printing options and trigger a print job to be executed.
   *
  ...
  /**
   * Controls the visibility of the developer console. The console displays various types of events that are fired by
   * the calendar while the user is using it.
   *
  ...
  /**
   * Property used to reference a node that can be used as a header for the calendar view. The skin of this view uses a
   * {@link BorderPane} to lay out its children. The header will be placed in its top location.
   *
  ...
  /**
   * Property used to reference a node that can be used as a footer for the calendar view. The skin of this view uses a
   * {@link BorderPane} to lay out its children. The footer will be placed in its bottom location.
   *
   ...
   *
   * @param transitions if true transitions will be used to go from one page to another
   */
   ...
   *
   * @param animated if true tray animations will be used when opening / closing the source tray or the search result
   *          tray
   */
  ...
  /**
   * Controls whether the search field (text field) in the upper right corner of the control will be shown to the user
   * or not.
   *
  ...
  /**
   * Controls whether the "add calendar" button in the upper left corner of the control will be shown to the user or
   * not.
   *
   ...
   *
   * @param show if true the "add calendar" button will be accessible by the user
   */
  ...
  /**
   * Controls whether the "page-specific" toolbar controls (e.g. DayPage: agenda view, day view, combined view) in the
   * upper left corner of the control will be shown to the user or not.
   *
   * @return true if the "page-specific" toolbar controls will be accessible by the user
   ...
   *
   * @param show if true the "page-specific" toolbar controls will be accessible by the user
   * @see PageBase#getToolBarControls()
   */
  ...
  /**
   * Sends the request to the calendar view to display the given date. The view will switch to the {@link DayPage} and
   * set the value of {@link #dateProperty()} to the date.
   *
  ...
  /**
   * Sends the request to the calendar view to display the given week. The view will try to switch to the
   * {@link WeekPage} and set the value of {@link #dateProperty()} to the date.
   *
  ...
  public final void showWeek(Year year, int weekOfYear) {
    ...
    setDate(LocalDate.of(year.getValue(), 1, 1)
        .plusWeeks(weekOfYear));
  }

  /**
   * Sends the request to the calendar view to display the given date and time. The view will switch to the
   * {@link DayPage} and set the value of {@link #dateProperty()} to the date and {@link #requestedTimeProperty()} to
   * the time.
   *
  ...
  /**
   * Sends the request to the calendar view to display the given year and month. The view will switch to the
   * {@link MonthPage} and set the value of {@link #dateProperty()} to the first day of the month.
   *
  ...
  /**
   * Sends the request to the calendar view to display the given year. The view will switch to the {@link YearPage} and
   * set the value of {@link #dateProperty()} to the first day of the year.
   *
